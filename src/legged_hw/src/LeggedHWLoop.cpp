  ...
  : nh_(nh), hardwareInterface_(std::move(hardware_interface)), loopRunning_(true)
{
  ...
  // 设置默认值，如果参数没有，就用这里的默认
  nhP.param("loop_frequency", loopHz_, 500.0);                  // 默认 500Hz
  nhP.param("cycle_time_error_threshold", cycleTimeErrorThreshold_, 0.002); // 默认 2ms
  nhP.param("thread_priority", threadPriority, 95);             // 默认 95

  ROS_INFO_STREAM("loop_frequency = " << loopHz_
                  << ", cycle_time_error_threshold = " << cycleTimeErrorThreshold_
